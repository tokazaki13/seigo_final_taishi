#出庫用出力以外も出力したいときがあるかもしれないのでこれは使わない
def get_order_data(order_id):
    ...
    sql = ("SELECT 【編集禁止】受注テーブル（一覧表示用）.id, 【編集禁止】受注テーブル（一覧表示用）.name, order_date, billing_amount, shipping_fee, itemcode, 【編集禁止】受注明細テーブル（一覧表示用）.name, ordernum, price, price_with_tax, total, total_with_tax, received, returned, delivered FROM 出庫待ちリスト WHERE order_id = %s")
